"""Server manager — detect, launch, and manage serve.py for tests.

Handles:
- serve.py: detect on port 8080, launch as subprocess, staleness restart
- Health checks: wait for service to respond before returning
"""
...
def ensure_services():
    """Boot serve.py if needed. Restart if stale.

    This is the main entry point. Call it before running any tests.
    Returns (None, serve_ok) tuple for backward compatibility.
    """
    print("\n== Service Manager ==")

    serve_ok = start_serve()
    if not serve_ok:
        print("  WARNING: serve.py not available — all browser tests will fail")

    print(f"  Services: serve.py={'UP' if serve_ok else 'DOWN'}")
    print()
    return None, serve_ok


def shutdown():
    """Stop serve.py if we launched it."""
    global _serve_process
